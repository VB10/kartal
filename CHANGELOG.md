...
- **BREAKING**: Migrated from deprecated `dart:html` to `package:web` for web support
- Updated `share_plus` API to use `SharePlus.instance.share()` instead of deprecated `Share.share()`
- Implemented web platform share methods (`share`, `shareMail`, `shareWhatsApp`) with clipboard and URL launcher support
- Added singleton Dio instance for `CustomLinkPreview` to improve performance
- Fixed web platform device utilities (`deviceId`, `initPackageInfo`, `shareMailText`)
- Improved `isAndroid` detection on web using user agent

- Fixed catch clauses to use 'on Exception' for better error handling
- Renamed enum values to follow lowerCamelCase convention
- Fixed typo: isNotNullOrNoEmpty → isNotNullOrNotEmpty
- Added documentation to StringShareMixin methods
- Added tests for bool_extension, map_extension, widget_extension, future_extension
- Removed unused dart:math import

### New Features (4.4.0)

- **Duration Extension**: Added `5.minutes`, `2.hours`, `1.days` syntax and `Duration.toHumanReadable()`, `timeAgo()`, `timeFromNow()` methods
- **Connectivity Extension**: Added `context.isOnline`, `context.isWifi`, `context.isCellular` for network status checking
- **Color Manipulation Extension**: Added `color.darken()`, `color.lighten()`, `color.contrast()`, `color.toHex()`, `color.invert()`, `color.isLight`, `color.isDark`, `color.mix()`, `color.toMaterialColor()`
- **String Locale Extension**: Added `string.slugify()`, `string.toTrLowerCase()`, `string.toTrUpperCase()`, `string.isValidHexColor`, `string.removeAccents`, `string.capitalizeFirst()`, `string.capitalizeWords()`
- **List Operations Extension**: Added `list.chunked()`, `list.groupBy()`, `list.sum()`, `list.average()`, `list.distinct()`, `list.firstOrNull`, `list.lastOrNull`, `list.firstWhereOrNull()`, `list.takeLast()`, `list.skipLast()`
- **Dialog Extension**: Added `context.showSnack()`, `context.showSuccessSnack()`, `context.showErrorSnack()`, `context.showConfirm()`, `context.showAlert()`, `context.showModalBottom()`
- **DateTime Relative Extension**: Added `dateTime.timeAgo`, `dateTime.isToday`, `dateTime.isYesterday`, `dateTime.isTomorrow`, `dateTime.isPast`, `dateTime.isFuture`, `dateTime.startOfDay`, `dateTime.endOfDay`, `dateTime.startOfWeek`, `dateTime.endOfWeek`, `dateTime.age`
- **Form Key Extension**: Added `formKey.validateOrFocus()`, `formKey.resetForm()`, `formKey.saveForm()`, `formKey.isValid`, `formKey.isInvalid`
- **Network Info Extension**: Added `context.wifiIP`, `context.wifiName`, `context.wifiBSSID`, `context.hostname`
- **Asset Path Extension**: Added `string.asAssetPath`, `string.asImagePath`, `string.asIconPath`, `string.asJsonPath`, `string.asSvgPath`, `string.asLottiePath`
- **String Mask Extension**: Added `string.maskEmail()`, `string.maskPhone()`, `string.maskCreditCard()`, `string.maskSensitive()`
- **Widget Animation Extension**: Added `widget.shake()`, `widget.fadeIn()`, `widget.scaleIn()`, `widget.pulse()`, `widget.bounceIn()`
- **Context Arguments Extension**: Added `context.routeArgs<T>()`, `context.routeName`, `context.routeSettings`, `context.isCurrentRouteFirst`, `context.isCurrentRouteActive`
- **Map Safe Access Extension**: Added `map.getValue<T>()`, `map.getString()`, `map.getInt()`, `map.getDouble()`, `map.getBool()`, `map.getList<T>()`, `map.getMap()`, `map.hasKey()`, `map.getValueOrDefault()`

### Dependencies Updated
- Added `connectivity_plus: ^6.1.1` for network connectivity checking
- Added `network_info_plus: ^6.1.1` for WiFi information
